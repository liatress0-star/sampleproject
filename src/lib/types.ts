...
export type DataSourceType = "table" | "query" | "procedure" | "external" | "sample";
...
export interface DataSourceConfig {
  ...
  procedureParams?: { name: string; type: string; defaultValue?: string }[];
  // external
  externalUrl?: string;
  externalMethod?: "GET" | "POST";
  externalHeaders?: string;  // JSON string
  // sample (내장 샘플 데이터)
...
export interface MenuConfig {
  ...
  layout: LayoutItem[];
  href?: string;             // 전용 라우트가 있는 메뉴 (샘플 페이지 등)
  createdAt: string;
  updatedAt: string;
